export class Positioning {
  public position(element: HTMLElement, round: boolean = true): ClientRect {
  ...
  public offset(element: HTMLElement, round: boolean = true): ClientRect {
  ...
  public positionElements(
    ...
    switch (placementPrimary) {
      case 'top':
        targetElPosition.top =
          hostElPosition.top -
          (targetElement.offsetHeight + parseFloat(targetElStyles.marginBottom || ''));
        break;
      case 'bottom':
    ...
    switch (placementSecondary) {
      ...
      case 'right':
        targetElPosition.left =
          hostElPosition.left + hostElPosition.width - targetElement.offsetWidth;
        break;
      case 'center':
  ...
  public getAvailablePlacements(hostElement: HTMLElement, targetElement: HTMLElement): string[] {
    ...
    const windowHeight = window.innerHeight || (html && html.clientHeight) || 0;
    const windowWidth = window.innerWidth || (html && html.clientWidth) || 0;
    ...
    if (targetElemClientRect.width < hostElemClientRect.left) {
      // check for left only
      if (
        hostElemClientRectVerCenter > targetElemClientRect.height / 2 &&
        windowHeight - hostElemClientRectVerCenter > targetElemClientRect.height / 2
      ) {
        availablePlacements.splice(availablePlacements.length, 1, 'left');
      }
      // check for left-top and left-bottom
      this.setSecondaryPlacementForLeftRight(
        hostElemClientRect,
        targetElemClientRect,
        'left',
        availablePlacements
      );
    }
    ...
    if (targetElemClientRect.height < hostElemClientRect.top) {
      if (
        hostElemClientRectHorCenter > targetElemClientRect.width / 2 &&
        windowWidth - hostElemClientRectHorCenter > targetElemClientRect.width / 2
      ) {
        availablePlacements.splice(availablePlacements.length, 1, 'top');
      }
      this.setSecondaryPlacementForTopBottom(
        hostElemClientRect,
        targetElemClientRect,
        'top',
        availablePlacements
  ...
  private setSecondaryPlacementForLeftRight(
    hostElemClientRect: ClientRect,
    targetElemClientRect: ClientRect,
    primaryPlacement: string,
    availablePlacementArr: string[]
  ) {
    const html = document.documentElement;
    ...
    if (html) {
      ...
      if (
        (window.innerHeight || html.clientHeight) - hostElemClientRect.top >=
        targetElemClientRect.height
      ) {
  ...
  private setSecondaryPlacementForTopBottom(
    ...
    if (html) {
      // check for left-bottom
      if (
        (window.innerWidth || html.clientWidth) - hostElemClientRect.left >=
        targetElemClientRect.width
      ) {
...
export const positionElements = (
  ...
  let topVal = 0;
  let leftVal = 0;
  let appliedPlacement: Placement = 'auto';
  ...
  for (const {item, index} of toItemIndexes(placementVals)) {
    ...
    if (
      availablePlacements.find(val => val === item) != null ||
      placementVals.length === index + 1
    ) {
  ...
  targetElement.style.top = `${topVal + topOffset}px`;
  targetElement.style.left = `${leftVal + leftOffset}px`;
